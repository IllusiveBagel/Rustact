+++
title = "Architecture & Runtime Guide"
description = "Understand every moving piece inside Rustact: components, hooks, context, runtime, renderer, and events."
weight = 30
template = "doc.html"
updated = 2025-11-21
+++
...
Rustact is a React-inspired framework for building asynchronous terminal UIs on top of [`ratatui`](https://github.com/ratatui-org/ratatui). This document explains components, hooks, context, runtime internals, the renderer, and the event system so you can confidently extend the framework or build your own apps.
...
Rustact components are plain functions that receive a mutable `Scope` and return an `Element`.
...
- `Element::vstack / hstack` to arrange children in rows/columns.
...
Hooks live in `src/hooks/mod.rs` and are tracked per component via `HookRegistry`. Ordering mirrors React: call hooks in the same order every render.
...
`EventBus` is a Tokio `broadcast::channel` shared across the runtime. Every keyboard, mouse, resize, and tick event is published as a `FrameworkEvent`. `Ctrl+C` is detected and triggers an app shutdown.
...
2. Enters an `mpsc::Receiver<AppMessage>` loop. On `RequestRender`:
    ...
    - Recursively traverses the `Element` tree, building `View` structs.
...
- Enters alternate screen, hides the cursor, enables mouse capture.
- Recursively renders views using `Layout` for flex nodes, `Paragraph` for text, `List`/`ListState` for feeds, `Gauge` for progress bars, `Table`/`Tree` widgets for structured data, and a custom `TextInputWidget` for focusable inputs.
