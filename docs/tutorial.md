...
This walkthrough creates a fresh Rustact-powered terminal dashboard from scratch. Expect to spend ~20 minutes.
...
Use the dispatcher’s event bus plus `is_button_click` helper:

```rust
use rustact::{FrameworkEvent, Dispatcher};
...
Congrats—you now have a custom Rustact-powered TUI! Expand it following the roadmap and reference documents in `docs/` whenever you need deeper detail.
